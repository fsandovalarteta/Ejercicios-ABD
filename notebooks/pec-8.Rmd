...
setwd("~/Desktop/Ejercicios-ABD/notebooks")
```
...
base_datos <- read.csv("base_datos.csv")
...
Por motivos del ejercicio, voy a utilizar el 80% de la muestra para un análisis factorial desde la perspectiva, que luego servirá de prior informativo para una nueva estimación utilizado el 20% sobrante de la muestra.
...
Otros autores como [Lopes (2003)](https://www.jstor.org/stable/43601025) plantean el uso de priors acorde a las posteriors esperadas, también llamadas expected-posterior priors (EPP) utilizando una muestra reducida de datos.
...
Siguiendo los ejemplos de [Depaoli (2021)](https://www.routledge.com/Bayesian-Structural-Equation-Modeling/Depaoli/p/book/9781462547746) podría utilizarse una varianza de 0.1.
...
modelo2 <- '
asesor =~ int1 + prior("normal(0.94,0.1)")*int2 + prior("normal(0.951,0.1)")*int3 + prior("normal(0.912,0.1)")*int4 + prior("normal(0.852,0.1)")*int5 + prior("normal(0.938,0.1)")*int6 + prior("normal(0.801,0.1)")*int7 + prior("normal(0.909,0.1)")*int8 + prior("normal(0.833,0.1)")*int9 + prior("normal(1.016,0.1)")*int10
materiales =~ int11 + prior("normal(0.877,0.1)")*int12 + prior("normal(0.874,0.1)")*int13 + prior("normal(0.751,0.1)")*int14 + prior("normal(0.914,0.1)")*int15 + prior("normal(0.882,0.1)")*int16 + prior("normal(0.956,0.1)")*int17 + prior("normal(0.927,0.1)")*int18 + prior("normal(0.851,0.1)")*int19 + prior("normal(0.708,0.1)")*int20
estudiantes =~ int21 + prior("normal(1.12,0.1)")*int22 + prior("normal(0.789,0.1)")*int23 + prior("normal(0.908,0.1)")*int24 + prior("normal(0.866,0.1)")*int25 + prior("normal(1.051,0.1)")*int26 + prior("normal(1.031,0.1)")*int27 + prior("normal(0.86,0.1)")*int28 + prior("normal(1.095,0.1)")*int29

asesor ~~ prior("normal(0.420,0.1)")*materiales
asesor ~~ prior("normal(0.273,0.1)")*estudiantes
materiales ~~ prior("normal(0.290,0.1)")*estudiantes
'
